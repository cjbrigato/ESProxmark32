...
#include <WiFiType.h>
#include <esp_wifi.h>
#include "cjbVT100/cjbVT100.h"

class MHeader {
public:
  MHeader() {};

  uint16_t colorFill = 0x0000;
  ...
  char * scrollMode(){
    if (get_scroll_mode())return "SCROLL-ON";
    return "SCROLL-OFF";
  }
  ...
  void draw()
  {
    ...
    M5.Lcd.setCursor(M5.Lcd.width() - 112, 0);
    M5.Lcd.printf("Free %7d Byte", esp_get_free_heap_size());
    M5.Lcd.fillRect(x, 0, M5.Lcd.width() - 112-x, 8, colorFill);
    #ifdef SCROLL_DEBUG
    M5.Lcd.setCursor(M5.Lcd.width() - 196, 0);
    M5.Lcd.printf("%s",scrollMode());
    #endif
  }
};
