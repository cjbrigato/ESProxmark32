...
#include "hf_colin_esp32.h"
#include "hf_colin_esp32_appmain.h"
...
//----------------------------
    ...
    DbprintfEx(FLAG_NEWLINE, "%s>>%s C.J.B's Interactive ESP32 Mode started\r\n", _XRED_, _XWHITE_);
...
failtag:
    ...
    while (!iso14443a_select_card(cjuid, &p_card, &cjcuid, true, 0, true)) {
        ...
        if (BUTTON_HELD(10) > 0 || data_available() ) {
            WDT_HIT();
